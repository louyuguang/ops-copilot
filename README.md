...
- Week6 Day6（workflow 最小场景回归，保持 MVP）
  - 新增最小 scenario regression 用例，覆盖 retrieve/checks/final synthesis 在主路径与降级路径的关键语义
  - 固化 4 类可重复场景：全 primary、retrieve 失败 continue、checks 失败 fallback、final synthesis 失败 fallback
  - 断言聚焦 `workflow_trace` / `path_decision` / `workflow_overview`，确保行为可解释且可回归
- Week7 Day2（最小 observability 字段，保持 MVP）
  - 每次 pipeline run 自动生成 `request_id`（16 字符 hex，写入 `last_run_metadata`）
  - pipeline 级 metadata 新增 `total_duration_ms`（整次 run 耗时，毫秒）
  - workflow trace 中每个执行步骤（retrieve / checks / final_analysis）新增 `duration_ms`（毫秒）
  - `incident` 步骤为标记步骤，无 `duration_ms`
  - 关系：`total_duration_ms` >= 各步骤 `duration_ms` 之和（差值为 pipeline 自身开销）
- Week7 Day3（token/cost 最小观测接入，保持 MVP）
  - pipeline 级 metadata 新增统一 `token_usage`：`prompt_tokens` / `completion_tokens` / `total_tokens`
  - 新增 `token_usage_available` 显式可用性标记；无 usage 时会给出 `reason`（如 `provider_usage_missing` / `llm_not_called` / `llm_not_used`）
  - pipeline 级 metadata 新增 `cost_estimate`：`input_cost` / `output_cost` / `total_cost` + `estimate_basis`
  - 无 usage 或未走 LLM 时不会“静默缺失”，而是明确 `cost_estimate.available=false` 并保留解释信息
- Week7 Day4（error/degraded 最小收口，保持 MVP）
  - pipeline 级 metadata 新增 `error_summary`：统一表达 `had_error`、主错误类型与主要出错步骤
  - pipeline 级 metadata 新增 `degraded_reason`：统一表达 run 为 `fallback/continue/clean_run` 的核心原因与步骤
  - 与现有 `run_status` / `path_decision` / fallback 语义对齐，兼容原有 metadata 字段
- Week7 Day5（metadata 命名/边界小收口，保持 MVP）
  - 新增 `observability` 视图，按 `run / workflow / steps / resources` 聚合最小观测字段
  - 保留顶层字段（如 `workflow_trace` / `workflow_overview` / `token_usage` / `cost_estimate` / `error_summary` / `degraded_reason`）以保证兼容
  - 收敛 `checks` 与 `structured_checks` 的命名歧义：`checks` 作为主字段，`structured_checks` 保留为兼容别名
- Week7 Day6（observability 最小场景测试回归，保持 MVP）
  - 增加 clean / degraded fallback / empty retrieval continue / no-usage 场景断言
  - 断言以行为语义为中心（状态、路径决策、原因与 observability 视图一致性），不只校验字段存在
- Prompt 模板从代码抽离到文件：
