...
- **能力降级（fallback，不中断主流程）**
  - `ANALYSIS_MODE=llm` 但缺 `OPENAI_API_KEY`：自动回退到 rule 分析器，debug metadata 中 `fallback_reason=llm_api_key_missing`
  - `RETRIEVER_MODE=chroma` 但 Chroma 服务不可达：自动回退到 local retriever，metadata 中 `fallback=true`
