...
# 4) 一键对比报告（输出 local/chroma 差异 + retriever metadata）
python scripts/compare_retrievers.py
```

`compare_retrievers.py` 现在会按样本输出可读 diff：
...
建议阅读顺序：
1. 先看 `recommended_refs` 是否有明显差异（最直接反映检索差异）
2. 再看 `possible_causes/suggested_checks` 是否发生变化（检索是否影响推理）
3. 最后看 metadata（差异是因为 `top_k`、context 规模、还是 fallback）

脚本结尾会附 `JSON_REPORT`，便于后续做机器处理/留档。
